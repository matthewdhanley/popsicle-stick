...
__global__ void monte_carlo_kernel(int* grid, int* draws, int m, curandState* state, double* average){
...
//    int n_blocks = (N + threads_per_block - 1) / threads_per_block;
    ...
    monte_carlo_kernel<<<n_blocks, threads_per_block>>>(d_grid, d_draws, m, d_state, d_average);
    cudaCheckError();
    cudaSafeCall( cudaDeviceSynchronize() );
